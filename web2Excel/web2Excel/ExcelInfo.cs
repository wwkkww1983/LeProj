...
namespace web2Excel
{
    class ExcelInfo
    {
        ...
        {
            string[] headItems = { "项目名称", "地址", "幢号（楼号）", "层高", "所在层", "房间号", "户型", "用途", "建筑面积", "套内面积", "分摊面积", "房屋状态", "预售/现售", "房屋单价", "成交日期" };
            int idx = 1;
        ...
        public static ErrorInfo CompareData(string todayFileName, string yestdayFileName, string dealDate, List<string> projComparedList)
        {
            ...
            for (int i = 0; i < todayRow - 1; i++)//减去第一行标题
            {
                ...
                try
                {
                    ...
                    //bool tmpCheckFlag = false;
                    //if (todayExcelData[i, 0] == "中邦城市花园二期B区" && todayExcelData[i, 2] == "1栋" && todayExcelData[i, 4] == "1单元" && todayExcelData[i, 5] == "2604房")
                    //{
                    //    tmpCheckFlag = true;
