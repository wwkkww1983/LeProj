...
namespace XczdServer
{
    ...
    {
        private const int SERVER_PORT_NUMBER = 45556;
        ...
        private static int getCharValue(char asciiChar)
        {
            return asciiChar > 'a' ? asciiChar - 'a' + 10 : asciiChar > 'A' ? asciiChar - 'A' + 10 : asciiChar - '0';
        }

        public static void ProtocolExplain()
        {
            //byte[] tmpByte = new byte[] { 0x19, 0x00, 0x0B, 0x31, 0x37, 0x32, 0x38, 0x30, 0x36, 0x2D, 0x30, 0x30, 0x31, 0x34, 0x01, 0x05, 0x31, 0x32, 0x34, 0x35, 0x31, 0x06, 0xD6, 0xDC, 0xD0, 0xA1, 0xC4, 0xDD, 0x01, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
            string strByte = "0x19000B3137323830362D303031340105313234353106D6DCD0A1C4DD0109000000000000000000000000000000000000000000000000000000";
            byte[] byteInfo = new byte[strByte.Length / 2 - 1];
            for (int i = 2, byteIdx = 0; i < strByte.Length; i += 2, byteIdx++)
            {
                byteInfo[byteIdx] = (byte)((getCharValue(strByte[i])) << 4 | (getCharValue(strByte[i + 1])));
            }
            ClientHanlderProductInfo clientHandler = new ClientHanlderProductInfo();
            clientHandler.HandlerClientData(byteInfo);
        }
