<template>
  <div class="setDepManager">
    <p>当前部门主管：<span v-if="current">{{current.nickName}}</span><span v-else>未设置</span></p>
...
<script>
  import {depStaff, setDepManager} from '../../../Api/department'
    ...
    methods: {
      ...
      setManager: function() {
        ...
        let data = {
          ...
          type: _self.type ? _self.type : '3'
        }
        setDepManager(data).then((res) => {
          console.log(res)
        })
      }
    },
