            ...
            <div class="main-box">
                <div class="sipping-header">
                    <ul class="sipping-nav">
                        <router-link tag="li" to="/work/StorageReturn">退库单据<span>&nbsp>&nbsp</span></router-link>
                    ...
                    <ul class="header-btns">
                        ...
                        <li v-if="jurisdiction.single" @click="submitAudit">提交审核</li>
                        <li v-if="jurisdiction.reviewer" @click="passAudit">通过</li>
                        <li v-if="jurisdiction.reviewer" @click="refuseAudit">驳回</li>
                        <li v-if="jurisdiction.reviewer" @click="cancelAudit">撤销审核</li>
                        <li v-if="jurisdiction.consignee">确认收货</li>
            ...
            <header class="header-box">
                <div class="header-seek mt20">
                    <ul class="header-operation-btn">
                        ...
                        <li>
                            <span>调出库位：<span class="main-color" v-if="receiptDetail">{{receiptDetail.outWarehouse}}</span></span>
                        </li>
                        <li>
                            <span>商品属性：<span class="main-color" v-if="receiptDetail">{{getproductTpye(receiptDetail.productTpye)}}</span></span>
                        </li>
                        <li>
                            <span>调入库位：<span class="main-color" v-if="receiptDetail">{{receiptDetail.warehouse}}</span></span>
                        </li>
                        <li>
                            <span>备注：</span>
...
import {seekProductDetail, seekReceiptTKSynopsis, seekReceiptRemark} from './../../../Api/commonality/seek'
import plainStorageReturn from "./../CommonalityComponent/billDetials/PlainStorageReturn"
// import remarkTemplate from "./../CommonalityComponent/RemarkTemplate"
export default {
    data () {
        return {
            pickerOptions0: {
            ...
            receiptDescription: "",
            receiptRemark: "",
            orderRemarkList: null
        }
    },
    components: {
        affirm, error, plainStorageReturn
    },
    computed: {
        ...mapGetters([
            "shopLists", // 分销商
    ...
    methods: {
        ...
        //                 "applyType": "2", // 1 调库 2 退库 3 修改 4 入库 5 收货 6 发货 7 退货 8 调柜（销售除外）
        ...
        addSingleAll () { // 保存，目前不做分页保存
            ...
            let data = {
                ...
                "type": "2" // 单据类型:1 调库 2 退库 3 修改 4 入库 5 收货 6 发货 7 退货 8 调柜
            }
            sessionStorage.setItem("xiao华", JSON.stringify(data));
            this.receiptAddOrCheck(data);
        },
        close () { // 关闭
            this.$router.push("/work/transferStorage");
        },
        getAjax () { // 权限设置
            ...
            if (this.receiptDescription.isCheckOrderMan !== "N") {
                this.jurisdiction.reviewer = true;
            }
            // if (this.receiptDescription.isCheckOrderMan !== "N") {
            //     this.jurisdiction.consignee = true;
            // }
            console.log("我的天，要疯了")
