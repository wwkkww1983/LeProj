                    ...
                    <div class="header-select-wrap">
                                ...
                                <ul>
                                    <template v-for="(item, index) in repositoryList">                          
                                        <li @click="getSort(item, index)">
                                            <span :class="{on: selectData.warehouseOut[index]}">
                                                {{item.repositoryName}}
                                            </span>
                                        </li>
                                    </template>
                        ...
                        <div class="one-line-data">
                            ...
                            <div class="one-line-right">
                                ...
                                <ul>
                                    <template v-for="(item, index) in datas.productTypeList">                 
                                        <li @click="getsupplier(item, index)">
                                            <span :class="{on: selectData.productType[index]}">{{item.name}}</span>
                                        </li>
                                    </template>
                                </ul>
                            </div>
            ...
            <div class="header-options-bottom" v-if="this.queryList.length > 0">
                ...
                <ul>
                    <li :class="{'li-on': selectData.littleSelect[index]}" v-on:mouseover="openList(index)" v-for="(item, index) in queryList" v-on:mouseleave="outList(index)">
                        ...
                        <template v-if='item.typeName === "调入库位"'>
                        ...
                        <template v-if='item.typeName === "调出库位"' v-on:mouseover="openList(index)">
                        ...
                        <template v-if='item.typeName === "商品属性"'>
                            <div v-on:mouseover="openList(index)" class="open-list" :class="{on: selectData.littleSelect[index]}" v-if="selectData.littleSelect[index]">
                                <template v-for="(item2, index2) in datas.productTypeList">                                      
...
<!--                         <a href="javascript: void(0)" @click="auditFun(item.checkType, item.orderNum)">
                        ...
                        <a class="del-bg del-btn" v-if='item.receiptStatus === "1"' href="javascript: void(0)" @click="delectReceipts(item.orderNum)">
                            删除
                        </a>
                    </div>
                </div>
...
export default {
    data () {
        return {
            ...
            "popup": { // 弹窗
                ...
                "supplierAnRepository": false, // 商品属性和库位
    ...
    methods: {
        ...
        filterFun (cbFun) { // 定位选择的过滤数据
            ...
            if (this.selectObject.productType) { // 商品属性
