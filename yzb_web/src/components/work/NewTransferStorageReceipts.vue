<template>
    <el-dialog class="dialog-w540-h320-hn" title="" v-model="isShow">
        ...
        <div class="list" v-if="cutData.one">
            <p class="popTitle">
                选择出库库位
                <a v-if="newDatas.repositoryOutId" href="javascript:void(0)" class="skip" @click="toPageFun('two', 'one')">下一步</a>
            </p>
            <ul>
            <!-- newDatas.repositoryId === item.repositoryId -->
                <li :class="{on: newDatas.repositoryOutId === item.repositoryId}" v-for="item in repositoryList" @click="getRepositoryOutId(item.repositoryId)">
                    {{item.repositoryName}}
                </li>
            </ul>
        </div>
