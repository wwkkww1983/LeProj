                ...
                <div class="el-tree">
                    <div class="el-tree-node" v-for="propData in propListData" :class="{'is-expanded': propData.isExpanded}">
                        <div class="el-tree-node__content" style="padding-left: 0px;" @click="propData.isExpanded = !propData.isExpanded">
                            <span class="el-tree-node__expand-icon" :class="{expanded: propData.isExpanded}"></span><span class="el-tree-node__label">{{ propData.prop.name }}</span>
                        </div>
                        <div class="el-tree-node__children" v-show="propData.isExpanded">
                            <div class="el-tree-node" v-for="sub_prop in propData.prop.subList">
                                <div class="el-tree-node__content" style="padding-left: 16px;" @dblclick="handlePropClick(sub_prop)" @mousedown="setComponentDragStartData($event, 'addProp', sub_prop)">
                                    <span class="el-tree-node__expand-icon is-leaf"></span><span class="el-tree-node__label">{{ sub_prop.name }}</span>
                                </div>
            ...
            <div class="canvas-panel">
                <div class="canvas-container" :style="canvasContainerStyle">
                    ...
                    <div class="canvas" ref="canvas" :style="canvasStyle" @mousedown="canvasMousedownHandler" @mousemove.prevent="canvasMousemoveHandler" @mouseup.prevent="canvasMouseupHandler">
                        ...
                        <div class="bounding-rect" v-show="isBounding" :style="boundingRectStyle"></div>
                    </div>
                </div>
            </div>
        </div>
...
import ImageSettingComponent from './ImageSetting'
import LineSettingComponent from './LineSetting'
import ContainerSettingComponent from './ContainerSetting'
import PropertySettingComponent from './PropertySetting'
...
export default {
        ...
        computed: {
            ...
            setting() {
                return {
                    ...
                    isLineComponent: this.activeComponents.length == 1 && this.activeComponents[0].type == 'LineComponent',
            ...
            boundingRectStyle() {
                if (this.isBounding && this.startX && this.startY && this.endX && this.endY) {
                    return {
                        top: Math.min(this.startY, this.endY) + 'mm',
                        left: Math.min(this.startX, this.endX) + 'mm',
                        width: Math.abs(this.endX - this.startX) + 'mm',
                        height: Math.abs(this.endY - this.startY) + 'mm'
                    }
                }
            }
        },
        ...
        methods: {
            ...
            editorMouseupHandler(e) {
                if (this.componentDragStartData._isDraging) {
                    let {offsetX, offsetY} = this._getCanvasOffset(e)
                    let left = Math.floor(offsetX)
                    let top = Math.floor(offsetY)
            ...
            _getCanvasOffset(e) {
                switch (this.canvas.rotateDeg) {
                    case 0:
                        return {
                            offsetX: (e.clientX - this.$refs.canvas.getBoundingClientRect().left) / this.ppi * 2.54 * 10 / (this.canvas.percentage / 100),
                            offsetY: (e.clientY - this.$refs.canvas.getBoundingClientRect().top) / this.ppi * 2.54 * 10 / (this.canvas.percentage / 100)
                        }
                    case 90:
                        return {
                            offsetX: (e.clientY - this.$refs.canvas.getBoundingClientRect().top) / this.ppi * 2.54 * 10 / (this.canvas.percentage / 100),
                            offsetY: this.canvas.height - (e.clientX - this.$refs.canvas.getBoundingClientRect().left) / this.ppi * 2.54 * 10 / (this.canvas.percentage / 100)
                        }
                    case 180:
                        return {
                            offsetX: this.canvas.width - (e.clientX - this.$refs.canvas.getBoundingClientRect().left) / this.ppi * 2.54 * 10 / (this.canvas.percentage / 100),
                            offsetY: this.canvas.height - (e.clientY - this.$refs.canvas.getBoundingClientRect().top) / this.ppi * 2.54 * 10 / (this.canvas.percentage / 100)
                        }
                    case 270:
                        return {
                            offsetX: this.canvas.width - (e.clientY - this.$refs.canvas.getBoundingClientRect().top) / this.ppi * 2.54 * 10 / (this.canvas.percentage / 100),
                            offsetY: (e.clientX - this.$refs.canvas.getBoundingClientRect().left) / this.ppi * 2.54 * 10 / (this.canvas.percentage / 100)
                        }
                }
            },
            //记录鼠标按下时的位置和选中的组件
            canvasMousedownHandler(e) {
                if (e.which == 1) {
                    ...
                    if (e.target.className == 'resize') {
                        offsetX -= 3
                        offsetY -= 3
                        this._resizingComponent = this._getComponentByPos({
                            offsetX,
                            offsetY
                        }, true)
                    } else {
                        this._dragingComponent = this._getComponentByPos({
                            offsetX,
                            offsetY
                        }, true)
                    }
            ...
            canvasMousemoveHandler(e) {
                if (e.which == 1) {
                    ...
                    this.endX = offsetX
                    this.endY = offsetY
                    if (this._dragingComponent || this._resizingComponent) {
                        let dx = Math.floor(this.endX - this.startX)
                        let dy = Math.floor(this.endY - this.startY)
                        if (this._dragingComponent) {
                            this._isDraging = true
                            this.moveComponent(this._dragingComponent, {
                                left: dx,
                                top: dy
                            })
                        }
                ...
                if (e.which == 1) {
                    if (this._isDraging || this._isResizing) {
                        ...
                        if (this.isBounding) {
                            ...
                            let interactComponents = []
                            this.canvas.components.forEach(component => {
                                if (isInteractWithComponent(boundingRect, component.data)) {
                                    interactComponents.push(component)
                                }
                            })
                            this.resetComponentsActiveState()
                            interactComponents.forEach(component => {
                                this.activeComponent(component, false)
                            })
                        } else {
                            if (e.target.className == 'resize') {
                                offsetX -= 3
                            ...
                            let clickingComponent = this._getComponentByPos({
                                offsetX,
                                offsetY
                            }, false)
            ...
            _getComponentByPos({
                ...
                components.forEach(component => {
                    let {
                        ...
                        right,
                        bottom
                    } = getComponentBound(component.data)
                        ...
                        if (component.type == 'ContainerComponent' && !isActive && component.active) {
                            component.data.children.forEach(child => {
                                let {
                                    ...
                                    right,
                                    bottom
                                } = getComponentBound(child.data, component.data)
                                if (offsetX > left && offsetX < right && offsetY > top && offsetY < bottom) {
                                    items.push(child)
                                }
                            })
            ...
            alignLeftBtnHandler() {
                if (this.menuItems.isAlignLeftAvailable) {
                    ...
                    this.activeComponents.forEach(component => {
                        this.moveComponentTo(component, {
                            left: left
                        })
                    })
                    this.record()
            ...
            alignCenterBtnHandler() {
                if (this.menuItems.isAlignCenterAvailable) {
                    ...
                    this.activeComponents.forEach(component => {
                        this.moveComponentTo(component, {
                            left: center - 0.5 * component.data.width
                        })
                    })
                    this.record()
                }
            ...
            alignRightBtnHandler() {
                if (this.menuItems.isAlignRightAvailable) {
                    ...
                    this.activeComponents.forEach(component => {
                        this.moveComponentTo(component, {
                            left: right - component.data.width
                        })
                    })
                    this.record()
                }
            ...
            alignMiddleBtnHandler() {
                if (this.menuItems.isAlignCenterAvailable) {
                    ...
                    this.activeComponents.forEach(component => {
                        this.moveComponentTo(component, {
                            top: middle - 0.5 * component.data.height
                        })
                    })
                    this.record()
                }
            ...
            alignBottomBtnHandler() {
                if (this.menuItems.isAlignBottomAvailable) {
                    ...
                    this.activeComponents.forEach(component => {
                        this.moveComponentTo(component, {
                            top: bottom - component.data.height
                        })
                    })
                    this.record()
                }
            ...
            alignVCenterBtnHandler() {
                if (this.menuItems.isAlignVCenterAvailable) {
                    ...
                    componentDataTemp.forEach((component, i) => {
                        this.moveComponentTo(component, {
                            top: firstMiddle + i * gap - 0.5 * component.data.height
                        })
                    })
                    this.record()
                }
            ...
            alignHCenterBtnHandler() {
                if (this.menuItems.isAlignHCenterAvailable) {
                    ...
                    componentDataTemp.forEach((component, i) => {
                        this.moveComponentTo(component, {
                            left: firstCenter + i * gap - 0.5 * component.data.width
                        })
                    })
                    this.record()
                }
                ...
                if (this.menuItems.isCombineAvailable) {
                    ...
                    this.canvas.components.forEach(component => {
                        if (component.active) {
                            ...
                            if (component.type == 'ContainerComponent') {
                                let activeChildren = component.data.children.filter(child => child.active)
            ...
            showItemSetting(component) {
                if (this.setting.isTextComponent || this.setting.isImageComponent || this.setting.isLineComponent || this.setting.isPropertyComponent || this.setting.isItemListComponent || this.setting.isContainerComponent) {
                    switch (component.type) {
                        ...
                        case 'ImageComponent':
                            ...
                            break
                        case 'LineComponent':
                            this.$refs.lineSettingComponent.$emit('set_data', component.data)
                            break
            ...
            changeComponentSetting(data) {
                this.changeComponentData(this.activeComponents[0], {
                    data,
                    shouldUpdate: false
                })
                this.record()
            ...
            removeComponent(component) {
                ...
                if (~index) {
                    ...
                    this.canvas.components = this.canvas.components.map(item => {
                        if (item.type === 'ContainerComponent') {
                            index = item.data.children.indexOf(component)
                            if (~index) {
                                item.data.children.splice(index, 1)
                            }
            ...
            updateItemListId() {
                ...
                itemListComponents.forEach(itemListComponent => {
                    ...
                    let alignNumber = itemListComponent.data.height / (itemListComponent.data.number + 1)
                    children.forEach(child => {
                        this.moveComponentTo(child, {top: Math.round(itemListComponent.data.top + alignNumber - 0.5 * child.data.height)})
                    })
...
.template-editor {
        ...
        .canvas-panel {
            ...
            .canvas-container {
                ...
                .left-ruler {
                    ...
                    bottom: 0;
                    width: 30px;
                    background-image: url(~assets/img/template/ruler-left.png);
                ...
                .top-ruler {
                    ...
                    right: 0;
                    height: 30px;
                    background-image: url(~assets/img/template/ruler-top.png);
                    background-repeat: no-repeat;
                    z-index: 100;
                }
                .canvas {
    ...
    .text-preview, .image-preview, .line-preview, .property-preview, .itemlist-preview {
        position: fixed;
        opacity: .7;
    }
    ...
    .image-preview {
        ...
        width: 85px;
        height: 68px;
        background-image: url(~assets/img/template/image-sample.png);
